...
export const createMajor = async (req, res) => {
  try {
    ...
    await db.execute("INSERT INTO majors (code, name, description) VALUES (?, ?, ?)", [code, name, description]);
    res.json({ message: "Major created successfully." });
  } catch (error) {
    console.error(error);
  ...
  try {
    ...
    await db.execute(
      ...
      [
        ...
        studySemester,
        programTrack || null,
        Boolean(isYear1Input),
        Boolean(isUsedInAlgorithm),
        Number(displayOrder || 999),
      ]
    );
...
export const createLearningPath = async (req, res) => {
  try {
    ...
    await db.execute(
      "INSERT INTO major_learning_paths (major_id, year_label, content) VALUES (?, ?, ?)",
      [majorId, yearLabel, content]
    );
...
export const getWeightsByMajor = async (req, res) => {
  try {
    ...
    const [foundation] = await db.execute(
      `SELECT mfr.id, s.name, s.subject_code, mfr.weight
    ...
    const [interests] = await db.execute(
      `SELECT imw.id, i.name, imw.weight
    ...
    const [selfAssessments] = await db.execute(
      `SELECT qmw.id, q.question_text, qmw.weight
    ...
    res.json({ foundation, interests, selfAssessments });
  } catch (error) {
    console.error(error);
